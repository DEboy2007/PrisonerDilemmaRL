...
class OpponentHistory:
    ...
    Can optionally start with optimistic prior: assumes opponent shares 80% of the time.
    """
    def __init__(self, initial_share_rate=0.5):
        self.history = []  # Stores all moves (0 for share, 1 for steal)
        self.initial_share_rate = initial_share_rate
        self.initial_steal_rate = 1.0 - initial_share_rate
    ...
    def get_state_vector(self):
        ...
        - Bin 0 (most recent 1/5th): steal_rate × 5
        - Bin 1 (next 1/5th): steal_rate × 4
        - Bin 2 (middle 1/5th): steal_rate × 3
        - Bin 3 (older 1/5th): steal_rate × 2
        - Bin 4 (oldest 1/5th): steal_rate × 1

        If no history, uses initial prior.
        ...
        if len(self.history) == 0:
            ...
            for bin_idx in range(5):
                weight = 5 - bin_idx
                state[bin_idx] = self.initial_steal_rate * weight
            return state
    ...
    def analyze_opponent_pattern(self):
        ...
        if len(self.history) == 0:
            return {
                'share_rate': self.initial_share_rate,
                'steal_rate': self.initial_steal_rate,
                'avg_my_reward': 0,
                'avg_opp_reward': 0,
                'total_rounds': 0
            }
        ...
        return {
            'share_rate': shares / total if total > 0 else self.initial_share_rate,
            'steal_rate': steals / total if total > 0 else self.initial_steal_rate,
...
class DQNPrisoner:
    ...
    Can start with optimistic or neutral belief about opponent.
    """
    def __init__(self, learning_rate=0.001, discount_factor=0.9, initial_epsilon=0.5, optimistic_prior=0.5):
        self.net_money = 0
        ...
        # Track opponent's history with specified prior
        self.opponent_history = OpponentHistory(initial_share_rate=optimistic_prior)
...
if __name__ == "__main__":
    print("=" * 70)
    print("DQN Prisoner's Dilemma - Asymmetric Trust")
    print("=" * 70)

    # Create asymmetric prisoners: P1 is optimistic (80% share belief), P2 is neutral (50% share belief)
    p1 = DQNPrisoner(learning_rate=0.001, discount_factor=0.9, initial_epsilon=0.5, optimistic_prior=0.8)
    p2 = DQNPrisoner(learning_rate=0.001, discount_factor=0.9, initial_epsilon=0.5, optimistic_prior=0.5)
    ...
    print(f"Weighting: [Most recent 1/5 × 5, Next 1/5 × 4, Middle 1/5 × 3, Older 1/5 × 2, Oldest 1/5 × 1]")
    print(f"**ASYMMETRIC SETUP:**")
    print(f"  - P1 starts OPTIMISTIC: believes opponent shares 80% of time")
    print(f"  - P2 starts NEUTRAL: believes opponent shares 50% of time\n")
    ...
    print(f"{'=' * 70}")
    print(f"P1 total money: {p1.net_money} (OPTIMISTIC)")
    print(f"P2 total money: {p2.net_money} (NEUTRAL)")
