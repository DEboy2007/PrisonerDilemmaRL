...
class DQNPrisoner:
    ...
    Uses 5-round lookahead rewards: each decision's reward is the sum of the next 5 rounds.
    This teaches longer-term consequences of decisions.
    """
    ...
    def update_q_value(self, my_reward, opponent_reward, opponent_move):
        """
        Updates Q-network using 5-round lookahead rewards.
        Each decision's reward = sum of next 5 rounds, showing medium-term consequences.
...
class TitForTatPrisoner:
    """
    Standard Tit-for-Tat strategy: Start with cooperation, then copy opponent's last move.
    NO forgiveness mechanism - pure Tit-for-Tat.
    This is the classic strategy from Axelrod's tournaments.
    """
    def __init__(self):
        self.net_money = 0
        self.opponent_history = OpponentHistory()
    ...
    def make_choice(self):
        """
        Makes a choice using standard Tit-for-Tat strategy.
        - First move: always share (cooperate)
        - Subsequent moves: copy opponent's last move exactly
        ...
        if self.last_opponent_move is None:
            ...
            # Copy opponent's last move exactly
...
if __name__ == "__main__":
    print("=" * 70)
    print("DQN vs Standard Tit-for-Tat (NO Forgiveness, 5-Round Lookahead)")
    print("=" * 70)

    print("\n" + "=" * 70)
    print("CONFIGURATION")
    print("=" * 70)
    print("\nStandard Tit-for-Tat Strategy:")
    print("  - First move: Always cooperate (share)")
    print("  - Every other move: Copy opponent's last move exactly")
    print("  - NO forgiveness mechanism")
    print("  - Pure, classic Tit-for-Tat from Axelrod's tournaments")
    print("\nDQN Agent:")
    print("  - 5-ROUND LOOKAHEAD rewards (sum of next 5 rounds)")
    print("  - Teaches medium-term consequences of decisions")
    print("  - Curiosity-driven epsilon (adaptive exploration)")

    # Create agents with 5-round lookahead
    dqn_agent = DQNPrisoner(learning_rate=0.001, discount_factor=0.9, initial_epsilon=1.0, lookahead_rounds=5)
    tft_agent = TitForTatPrisoner()

    # Phase 1: Random exploration (no learning)
    exploration_rounds = 1000
    print(f"\n" + "=" * 70)
    print(f"PHASE 1: EXPLORATION ({exploration_rounds} rounds)")
    print("=" * 70)
    ...
    for round_num in range(exploration_rounds):
        ...
        # Update agents (exploration phase - stores but doesn't train heavily)
    ...
    print("5-ROUND LOOKAHEAD: Each decision's reward = sum of next 5 rounds")
    print("This teaches the agent medium-term consequences of their choices!\n")
    ...
    if tft_analysis['share_rate'] > 0.7:
        print("\nDQN learned to cooperate! Despite no forgiveness from TFT,")
        print("the 5-round lookahead helped the agent understand that")
        print("cooperation leads to better long-term outcomes.")
    elif tft_analysis['steal_rate'] > 0.7:
        print("\nDQN exploited Tit-for-Tat. The agent tried to steal frequently,")
        print("and TFT retaliated by copying the stealing behavior.")
        print("WITHOUT forgiveness, they got stuck in mutual defection.")
        print("The 5-round lookahead was NOT sufficient to escape this trap.")
    else:
