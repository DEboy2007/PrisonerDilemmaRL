...
class DQNPrisoner:
    ...
    Standard 1-round rewards: each decision's reward is the current round only.
    """
    def __init__(self, learning_rate=0.001, discount_factor=0.9, initial_epsilon=0.5):
        ...
        self.lookahead_rounds = 1  # Standard: no lookahead
    ...
    def update_q_value(self, my_reward, opponent_reward, opponent_move):
        """
        Updates Q-network using standard 1-round rewards.
        Each decision's reward = current round only.
        ...
        # Store current state/action and reward for training
        if self.last_state is not None:
            next_state = self.opponent_history.get_state_vector()

            # Store in replay buffer with current round reward only
...
if __name__ == "__main__":
    print("=" * 70)
    print("DQN vs Standard Tit-for-Tat (NO Forgiveness, NO Lookahead)")
    print("=" * 70)
    ...
    print("\nDQN Agent:")
    print("  - Standard 1-round rewards (current round only)")
    print("  - NO lookahead mechanism")
    print("  - Curiosity-driven epsilon (adaptive exploration)")

    # Create agents
    dqn_agent = DQNPrisoner(learning_rate=0.001, discount_factor=0.9, initial_epsilon=1.0)
    ...
    print("Standard 1-round rewards: Each decision's reward = current round only")
    ...
    if tft_analysis['share_rate'] > 0.7:
        print("\nDQN learned to cooperate! When facing standard Tit-for-Tat,")
        print("the DQN agent discovered that sharing is more profitable.")
